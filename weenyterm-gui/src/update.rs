use crate::ICON_DATA;
use anyhow::anyhow;
use config::{configuration, weenyterm_version};
...
fn get_github_release_info(uri: &str) -> anyhow::Result<Release> {
    ...
    let _res = Request::new(&uri)
        ...
        .header("User-Agent", &format!("weeny/weenyterm-{}", weenyterm_version()))
        .send(&mut latest)
...
pub fn get_latest_release_info() -> anyhow::Result<Release> {
    get_github_release_info("https://api.github.com/repos/weeny/weenyterm/releases/latest")
}

#[allow(unused)]
pub fn get_nightly_release_info() -> anyhow::Result<Release> {
    get_github_release_info("https://api.github.com/repos/weeny/weenyterm/releases/tags/nightly")
}
...
fn set_banner_from_release_info(latest: &Release) {
    ...
    let url = format!(
        "https://weenyfurlong.org/weenyterm/changelog.html#{}",
        latest.tag_name
    );
...
fn update_checker() {
    ...
    loop {
        // Figure out which other weenyterm-guis are running.
        ...
        let socks = weenyterm_client::discovery::discover_gui_socks();

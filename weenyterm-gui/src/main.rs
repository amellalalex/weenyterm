...
use weenyterm_bidi::Direction;
use weenyterm_client::domain::ClientDomain;
use weenyterm_font::shaper::PresentationWidth;
use weenyterm_font::FontConfiguration;
use weenyterm_gui_subcommands::*;
use weenyterm_mux_server_impl::update_mux_domains;
use weenyterm_toast_notification::*;
...
#[derive(Debug, Parser)]
#[command(
    about = "Weeny's Terminal Emulator\nhttp://github.com/wez/weenyterm",
    version = config::weenyterm_version()
)]
struct Opt {
    /// Skip loading weenyterm.lua
    #[arg(long, short = 'n')]
    skip_config: bool,
...
enum SubCommand {
    ...
    #[command(name = "connect", about = "Connect to weenyterm multiplexer")]
...
async fn async_run_ssh(opts: SshCommand) -> anyhow::Result<()> {
    ...
    if opts.verbose {
        ssh_option.insert("weenyterm_ssh_verbose".to_string(), "true".to_string());
    }
...
async fn async_run_terminal_gui(
    ...
    weenyterm_blob_leases::register_storage(Arc::new(
        weenyterm_blob_leases::simple_tempdir::SimpleTempDir::new()?,
    ))?;
...
impl Publish {
    pub fn resolve(mux: &Arc<Mux>, config: &ConfigHandle, always_new_process: bool) -> Self {
        ...
        match weenyterm_client::discovery::resolve_gui_sock_path(
    ...
    pub fn try_spawn(
        ...
        if let Publish::TryPathOrPublish(gui_sock) = &self {
            ...
            match weenyterm_client::client::Client::new_unix_domain(None, &dom, false, &mut ui, true)
            {
                Ok(client) => {
                    ...
                    match res {
                        Ok(res) => {
                            log::info!(
                             ...
                             Use weenyterm start --always-new-process if you do not want this behavior. \
                             Result={:?}",
                                res
                            );
                            Ok(true)
...
fn spawn_mux_server(unix_socket_path: PathBuf, should_publish: bool) -> anyhow::Result<()> {
    let mut listener =
        weenyterm_mux_server_impl::local::LocalListener::with_domain(&config::UnixDomain {
    ...
    std::thread::spawn(move || {
        let name_holder;
        if should_publish {
            name_holder = weenyterm_client::discovery::publish_gui_sock_path(
                &unix_socket_path,
                &crate::termwindow::get_window_class(),
            );
...
fn run_terminal_gui(opts: StartCommand, default_domain_name: Option<String>) -> anyhow::Result<()> {
    ...
    // First, let's see if we can ask an already running weenyterm to do this.
...
fn notify_on_panic() {
    ...
    std::panic::set_hook(Box::new(move |info| {
        if let Some(s) = info.payload().downcast_ref::<&str>() {
            fatal_toast_notification("Weenyterm panic", s);
        }
        default_hook(info);
    }));
}
...
fn terminate_with_error_message(err: &str) -> ! {
    ...
    fatal_toast_notification("Weenyterm Error", &err);
    std::process::exit(1);
}
...
pub fn run_ls_fonts(config: config::ConfigHandle, cmd: &LsFontsCommand) -> anyhow::Result<()> {
    use weenyterm_font::parser::ParsedFont;
    ...
    let font_config = Rc::new(weenyterm_font::FontConfiguration::new(
        Some(config.clone()),
    ...
    if let Some(text) = &text {
        ...
        let ft_lib = weenyterm_font::ftwrap::Library::new()?;
            ...
            while let Some(info) = iter.next() {
                ...
                if config.custom_block_glyphs {
                    if let Some(block) = info.only_char.and_then(BlockKey::from_char) {
                        ...
                        println!(
                            "{:2} {:4} {:12} drawn by weenyterm because custom_block_glyphs=true: {:?}",
...
fn run() -> anyhow::Result<()> {
    ...
    #[cfg(windows)]
    {
        unsafe {
            ::windows::Win32::UI::Shell::SetCurrentProcessExplicitAppUserModelID(
                ::windows::core::PCWSTR(wide_string("org.weenyfurlong.weenyterm").as_ptr()),
            )
            .unwrap();
        }
    }
    ...
    let sub = match opts.cmd.as_ref().cloned() {
        ...
        None => {
            ...
            let mut argv = vec!["weenyterm-gui".to_string()];
    ...
    match sub {
        SubCommand::Start(start) => {
            ...
            weenyterm_blob_leases::clear_storage();
            res
        }

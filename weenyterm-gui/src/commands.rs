...
impl CommandDef {
    ...
    pub fn recreate_menubar(config: &ConfigHandle) {
        ...
        let weenyterm_perform_key_assignment_sel = sel!(weenytermPerformKeyAssignment:);
        ...
        let main_menu = match Menu::get_main_menu() {
            Some(existing) => {
                mark_candidates(
                    ...
                    weenyterm_perform_key_assignment_sel,
                );
        ...
        let mut order: Vec<&'static str> = vec!["WeenyTerm", "Shell", "Edit", "View", "Window"];
        ...
        for &title in &order {
            for cmd in &commands {
                ...
                let mut submenu = main_menu.get_or_create_sub_menu(&cmd.menubar[0], |menu| {
                    if cmd.menubar[0] == "Window" {
                    ...
                    } else if cmd.menubar[0] == "WeenyTerm" {
                        menu.assign_as_app_menu();

                        let about_item = MenuItem::new_with(
                            &format!("WeenyTerm {}", config::weenyterm_version()),
                            Some(weenyterm_perform_key_assignment_sel),
                            "",
                        );
                        ...
                        about_item.set_represented_item(RepresentedItem::KeyAssignment(
                            KeyAssignment::CopyTextTo {
                                text: config::weenyterm_version().to_string(),
...
pub fn derive_command_from_key_assignment(action: &KeyAssignment) -> Option<CommandDef> {
    Some(match action {
        ...
        QuitApplication => CommandDef {
            brief: "Quit WeenyTerm".into(),
            doc: "Quits WeenyTerm".into(),
        ...
        OpenUri(uri) => match uri.as_ref() {
            "https://weenyfurlong.org/weenyterm/" => CommandDef {
                brief: "Documentation".into(),
                doc: "Visit the weenyterm documentation website".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
                icon: Some("md_help"),
            },
            "https://github.com/wez/weenyterm/discussions/" => CommandDef {
                brief: "Discuss on GitHub".into(),
                doc: "Visit weenyterm's GitHub discussion".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
                icon: Some("oct_comment_discussion"),
            },
            "https://github.com/wez/weenyterm/issues/" => CommandDef {
                brief: "Search or report issue on GitHub".into(),
                doc: "Visit weenyterm's GitHub issues".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
                icon: Some("fa_ticket"),
...
fn compute_default_actions() -> Vec<KeyAssignment> {
    ...
    return vec![
        ...
        ShowTabNavigator,
        // ----------------- Help
        OpenUri("https://weenyfurlong.org/weenyterm/".to_string()),
        OpenUri("https://github.com/wez/weenyterm/discussions/".to_string()),
        OpenUri("https://github.com/wez/weenyterm/issues/".to_string()),
        ShowDebugOverlay,
        // ----------------- Misc

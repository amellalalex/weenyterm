...
pub fn show_debug_overlay(
    ...
    lua.load("weenyterm = require 'weenyterm'").exec()?;
    ...
    let version = config::weenyterm_version();
    let triple = config::weenyterm_target_triple();

    term.render(&[Change::Text(format!(
        "Debug Overlay\r\n\
         weenyterm version: {version} {triple}\r\n\

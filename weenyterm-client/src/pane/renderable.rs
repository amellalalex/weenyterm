...
use url::Url;
use weenyterm_term::{KeyCode, KeyModifiers, Line, StableRowIndex};
    ...
    pub fn get_lines(&self, lines: Range<StableRowIndex>) -> (StableRowIndex, Vec<Line>) {
        ...
        for idx in lines.clone() {
            ...
            if inner.client.overlay_lag_indicator && idx == inner.dimensions.physical_top {
                if inner.is_tardy() {
                    let status = format!(
                        "weenyterm: {:.0?}⏳since last response",
                    ...
                    let col = inner
                        .dimensions
                        .cols
                        .saturating_sub(weenyterm_term::unicode_column_width(&status, None));

...
#[error(
    "Please install the same version of weenyterm on both the client and server!\n\
...
impl Reconnectable {
    ...
    /// Resolve the path to weenyterm for the remote system.
    ...
    /// `path` comes from the SshDoman::remote_weenyterm_path option; if set
    ...
    /// message to the user.
    fn weenyterm_bin_path(path: &Option<String>) -> String {
        path.as_deref().unwrap_or("weenyterm").to_string()
    }
    ...
    fn ssh_connect(
        ...
        let proxy_bin = Self::weenyterm_bin_path(&ssh_dom.remote_weenyterm_path);
    ...
    pub fn tls_connect(
        ...
        if let Some(Ok(ssh_params)) = tls_client.ssh_parameters() {
            if self.tls_creds.is_none() {
                ...
                let mut ssh_config = weenyterm_ssh::Config::new();
                ...
                let creds = ui.run_and_log_error(|| {
                    ...
                    let cmd = format!(
                        "{} cli tlscreds",
                        Self::weenyterm_bin_path(&tls_client.remote_weenyterm_path)
                    );
        ...
        thread::spawn(move || {
            ...
            loop {
                if let Err(e) = client_thread(&mut reconnectable, local_domain_id, &mut receiver) {
                    ...
                    ui.title("weenyterm: Reconnecting...");
            ...
            .await
        {
            ...
            Err(err) => {
                ...
                let msg = if err.root_cause().is::<Timeout>() {
                    ...
                    format!(
                        "Please install the same version of weenyterm on both \

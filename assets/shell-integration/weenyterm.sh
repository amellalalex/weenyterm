...
# This file hooks up shell integration for weenyterm.
# It is suitable for zsh and bash.
#
# Although weenyterm is mentioned here, the sequences used are not weenyterm
...
__weenyterm_install_bash_prexec() {
...
} # end of __weenyterm_install_bash_prexec
...
# command provided by weenyterm if weenyterm is installed, but falls
# back to a simple printf command otherwise.
__weenyterm_osc7() {
  if hash weenyterm 2>/dev/null ; then
    weenyterm set-working-directory 2>/dev/null && return 0
    # If the command failed (perhaps the installed weenyterm
...
__weenyterm_semantic_precmd_executing=""
__weenyterm_semantic_precmd() {
  local ret="$?"
  if [[ "$__weenyterm_semantic_precmd_executing" != "0" ]] ; then
    __weenyterm_save_ps1="$PS1"
    __weenyterm_save_ps2="$PS2"
  ...
  if [[ "$__weenyterm_semantic_precmd_executing" != "" ]] ; then
    # Report last command status
    printf "\033]133;D;%s;aid=%s\007" "$ret" "$$"
  fi
  # Fresh line and start the prompt
  printf "\033]133;A;cl=m;aid=%s\007" "$$"
  __weenyterm_semantic_precmd_executing=0
}

function __weenyterm_semantic_preexec() {
  # Restore the original PS1/PS2
  PS1="$__weenyterm_save_ps1"
  PS2="$__weenyterm_save_ps2"
  # Indicate that the command output begins here
  printf "\033]133;C;\007"
  __weenyterm_semantic_precmd_executing=1
}

__weenyterm_user_vars_precmd() {
  __weenyterm_set_user_var "WEZTERM_PROG" ""
  __weenyterm_set_user_var "WEZTERM_USER" "$(id -un)"

  # Indicate whether this pane is running inside tmux or not
  if [[ -n "${TMUX}" ]] ; then
    __weenyterm_set_user_var "WEZTERM_IN_TMUX" "1"
  else
    __weenyterm_set_user_var "WEZTERM_IN_TMUX" "0"
  fi
  ...
  if [[ -z "${WEZTERM_HOSTNAME}" ]] ; then
    if hash hostname 2>/dev/null ; then
      __weenyterm_set_user_var "WEZTERM_HOST" "$(hostname)"
    elif hash hostnamectl 2>/dev/null ; then
      __weenyterm_set_user_var "WEZTERM_HOST" "$(hostnamectl hostname)"
    fi
  else
    __weenyterm_set_user_var "WEZTERM_HOST" "${WEZTERM_HOSTNAME}"
  fi
}

__weenyterm_user_vars_preexec() {
  # Tell weenyterm the full command that is being run
  __weenyterm_set_user_var "WEZTERM_PROG" "$1"
}
...
if [[ -z "${WEZTERM_SHELL_SKIP_SEMANTIC_ZONES}" ]]; then
  if [[ -n "$BLE_VERSION" ]]; then
    blehook PRECMD+=__weenyterm_semantic_precmd
    blehook PREEXEC+=__weenyterm_semantic_preexec
  else
    precmd_functions+=(__weenyterm_semantic_precmd)
    preexec_functions+=(__weenyterm_semantic_preexec)
  fi
fi

if [[ -z "${WEZTERM_SHELL_SKIP_USER_VARS}" ]]; then
  if [[ -n "$BLE_VERSION" ]]; then
    blehook PRECMD+=__weenyterm_user_vars_precmd
    blehook PREEXEC+=__weenyterm_user_vars_preexec
  else
    precmd_functions+=(__weenyterm_user_vars_precmd)
    preexec_functions+=(__weenyterm_user_vars_preexec)
  fi
fi

if [[ -z "${WEZTERM_SHELL_SKIP_CWD}" ]] ; then
  if [[ -n "$BLE_VERSION" ]]; then
    blehook PRECMD+=__weenyterm_osc7
  else
    precmd_functions+=(__weenyterm_osc7)
  fi
fi

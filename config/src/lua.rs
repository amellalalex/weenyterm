...
use std::path::Path;
use std::sync::Mutex;
use weenyterm_dynamic::{
    FromDynamic, FromDynamicOptions, ToDynamic, UnknownFieldAction, Value as DynValue,
};

pub use mlua;

static LUA_REGISTRY_USER_CALLBACK_COUNT: &str = "weenyterm-user-callback-count";
...
pub fn get_or_create_sub_module<'lua>(
    ...
    let sub = weenyterm_mod.get(name)?;
    match sub {
        Value::Nil => {
            let sub = lua.create_table()?;
            weenyterm_mod.set(name, sub.clone())?;
            Ok(sub)
        }
        Value::Table(sub) => Ok(sub),
        wat => anyhow::bail!(
            "cannot register module weenyterm.{name} as it is already set to a value of type {}",
            wat.type_name()
        ),
    }
}
        ...
        DynValue::Object(obj) => {
            match obj.get_by_str(&key) {
                None => {
                    ...
                    weenyterm_dynamic::Error::warn(message);
                }
                Some(_dvalue) => {
...
/// weenyterm specific config paths for lua modules, should
/// they choose to `require` additional code from their config.
///
/// A `weenyterm` module is registered so that the script can
/// `require "weenyterm"` and call into functions provided by
/// weenyterm.  The weenyterm module contains:
/// * `executable_dir` - the directory containing the weenyterm
///   executable.  This is potentially useful for portable
///   installs on Windows.
/// * `config_dir` - the directory containing the weenyterm
///   configuration.
/// * `log_error` - a function that logs to stderr (or the server
///   log file for daemonized weenyterm).
/// * `target_triple` - the rust compilation target triple.
/// * `version` - the version of the running weenyterm instance.
...
pub fn make_lua_context(config_file: &Path) -> anyhow::Result<Lua> {
    ...
    {
        let globals = lua.globals();
        // This table will be the `weenyterm` module in the script
        let weenyterm_mod = get_or_create_module(&lua, "weenyterm")?;
        ...
        prefix_path(&mut path_array, &crate::HOME_DIR.join(".weenyterm"));
        ...
        if let Ok(exe) = std::env::current_exe() {
            if let Some(path) = exe.parent() {
                weenyterm_mod
                    ...
                    .context("set weenyterm.executable_dir")?;
                if cfg!(windows) {
                    // For a portable windows install, force in this path ahead
                    // of the rest
                    prefix_path(&mut path_array, &path.join("weenyterm_modules"));
                }
            }
        }
...
package.searchers[2] = function(module)
  ...
  if name then
    package.loaded.weenyterm.add_to_config_reload_watch_list(name)
  end
  return orig(module)
end
        "#,
        ...
        weenyterm_mod
            ...
            .context("set weenyterm.config_file")?;
        weenyterm_mod
            .set(
                "config_dir",
                config_dir
            ...
            .context("set weenyterm.config_dir")?;

        lua.set_named_registry_value("weenyterm-watch-paths", Vec::<String>::new())?;
        weenyterm_mod.set(
            "add_to_config_reload_watch_list",
            lua.create_function(add_to_config_reload_watch_list)?,
        )?;

        weenyterm_mod.set("target_triple", crate::weenyterm_target_triple())?;
        weenyterm_mod.set("version", crate::weenyterm_version())?;
        weenyterm_mod.set("home_dir", crate::HOME_DIR.to_str())?;
        weenyterm_mod.set(
            "running_under_wsl",
        ...
        weenyterm_mod.set("font", lua.create_function(font)?)?;
        weenyterm_mod.set(
            "font_with_fallback",
            lua.create_function(font_with_fallback)?,
        )?;
        weenyterm_mod.set("hostname", lua.create_function(hostname)?)?;
        weenyterm_mod.set("action", luahelper::enumctor::Enum::<KeyAssignment>::new())?;
        weenyterm_mod.set(
            "has_action",
        ...
        weenyterm_mod.set("action_callback", lua.create_function(action_callback)?)?;
        weenyterm_mod.set("exec_domain", lua.create_function(exec_domain)?)?;

        weenyterm_mod.set("utf16_to_utf8", lua.create_function(utf16_to_utf8)?)?;
        weenyterm_mod.set("split_by_newlines", lua.create_function(split_by_newlines)?)?;
        weenyterm_mod.set("on", lua.create_function(register_event)?)?;
        weenyterm_mod.set("emit", lua.create_async_function(emit_event)?)?;
        weenyterm_mod.set("shell_join_args", lua.create_function(shell_join_args)?)?;
        weenyterm_mod.set("shell_quote_arg", lua.create_function(shell_quote_arg)?)?;
        weenyterm_mod.set("shell_split", lua.create_function(shell_split)?)?;
        ...
        // those values since weenyterm was started
...
/// <https://github.com/wez/weenyterm/issues/1736>
/// <https://github.com/wez/weenyterm/issues/1786>
...
///
/// `weenyterm.font("foo", {foreground="tomato"})`
/// yields:
...
///
/// `weenyterm.font_with_fallback({"Operator Mono", "DengXian"})`
///
/// The second optional argument is a list of other TextStyle fields,
/// as described by the `weenyterm.font` documentation.
...
/// This implements `weenyterm.on`, whose goal is to register an event handler
/// callback.
...
///
/// ```lua
/// weenyterm.on("event-name", function(arg1, arg2)
///   -- do something
///   return false -- if you want to prevent other handlers running
/// end);
///
/// weenyterm.emit("event-name", "foo", "bar");
/// ```
pub fn register_event<'lua>(
    lua: &'lua Lua,
    (name, func): (String, mlua::Function),
) -> mlua::Result<()> {
    let decorated_name = format!("weenyterm-event-{}", name);
...
const IS_EVENT: &str = "weenyterm-is-event-emission";
...
pub fn add_to_config_reload_watch_list<'lua>(
    ...
    let mut watch_paths: Vec<String> = lua.named_registry_value("weenyterm-watch-paths")?;
    watch_paths.extend_from_slice(&args);
    lua.set_named_registry_value("weenyterm-watch-paths", watch_paths)?;
    Ok(())
}
...
assert(weenyterm.emit('foo', 2) == false)

weenyterm.on('bar', function (n, str)
...
assert(weenyterm.emit('bar', 42, 'woot') == true)
"#,
            )
            .exec_async(),
        )?;

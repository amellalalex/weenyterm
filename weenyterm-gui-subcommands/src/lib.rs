...
pub const DEFAULT_WINDOW_CLASS: &str = "org.weenyfurlong.weenyterm";
...
pub struct StartCommand {
    ...
    /// in your weenyterm configuration file.
    #[arg(long = "no-auto-connect")]
    pub no_auto_connect: bool,

    /// If enabled, don't try to ask an existing weenyterm GUI instance
    /// to start the command.  Instead, always start the GUI in this
    /// invocation of weenyterm so that you can wait for the command
    /// to complete by waiting for this weenyterm process to finish.
    ...
    /// The default is "org.weenyfurlong.weenyterm".
    ...
    /// instance of weenyterm, including error, update and ssh
    ...
    /// weenyterm will simply attach and will NOT spawn the specified PROG.
    /// If you omit --attach when using --domain, weenyterm will attach
    /// AND then spawn PROG.
    #[arg(long, requires = "domain")]
    pub attach: bool,

    /// Instead of executing your shell, run PROG.
    /// For example: `weenyterm start -- bash -l` will spawn bash
...
pub struct SshCommand {
    ...
    /// `weenyterm ssh` is able to parse some (but not all!) options
    ...
    /// Instead of executing your shell, run PROG.
    /// For example: `weenyterm ssh user@host -- bash -l` will spawn bash

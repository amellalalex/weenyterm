...
pub fn register(lua: &Lua) -> anyhow::Result<()> {
    ...
    weenyterm_mod.set("nerdfonts", NerdFonts {})?;
    weenyterm_mod.set("format", lua.create_function(format)?)?;
    weenyterm_mod.set(
        "column_width",
    ...
    weenyterm_mod.set("permute_any_mods", lua.create_function(permute_any_mods)?)?;
    weenyterm_mod.set(
...
pub fn format_as_escapes(items: Vec<FormatItem>) -> anyhow::Result<String> {
    ...
    let mut renderer = new_weenyterm_terminfo_renderer();
...
fn permute_mods<'lua>(
    ...
    use weenyterm_input_types::Modifiers;
    ...
    static ref CAPS: Capabilities = {
        ...
        Capabilities::new_with_hints(
            ProbeHints::new_from_env()
                ...
                .colorterm(None)
                .colorterm_bce(None)
                .term_program(Some("WeenyTerm".into()))
                .term_program_version(Some(config::weenyterm_version().into())),
        )
        .expect("cannot fail to make internal Capabilities")
    };
}

pub fn new_weenyterm_terminfo_renderer() -> TerminfoRenderer {

...
use weenyterm_dynamic::{FromDynamic, Value as DynValue};
...
pub fn register(lua: &Lua) -> anyhow::Result<()> {
    ...
    weenyterm_mod.set("json_parse", lua.create_function(json_decode)?)?;
    weenyterm_mod.set("json_encode", lua.create_function(json_encode)?)?;
...
fn lua_value_to_json_value(value: LuaValue, visited: &mut HashSet<usize>) -> mlua::Result<JValue> {
    ...
    Ok(match value {
        ...
        LuaValue::UserData(ud) => match ud.get_metatable() {
            Ok(mt) => {
                if let Ok(to_dynamic) = mt.get::<mlua::Function>("__weenyterm_to_dynamic") {
                    match to_dynamic.call(LuaValue::UserData(ud.clone())) {
                        ...
                        Err(err) => {
                            return Err(mlua::Error::FromLuaConversionError {
                                from: "userdata",
                                to: "weenyterm_dynamic::Value",
                                message: Some(format!(
                                    "error calling __weenyterm_to_dynamic: {err:#}"
                                )),
                            })
                        }
                    }
                    ...
                    return Err(mlua::Error::FromLuaConversionError {
                        from: "userdata",
                        to: "weenyterm_dynamic::Value",
                        message: Some(format!("no __weenyterm_to_dynamic metadata")),
                    });
                }
            }
            Err(err) => {

...
/// by the user via `weenyterm.time.call_after`.
...
const SCHEDULED_EVENTS: &str = "weenyterm-scheduled-events";
...
pub fn register(lua: &Lua) -> anyhow::Result<()> {
    ...
    weenyterm_mod.set("sleep_ms", lua.create_async_function(sleep_ms)?)?;
    weenyterm_mod.set("strftime", lua.create_function(strftime)?)?;
    weenyterm_mod.set("strftime_utc", lua.create_function(strftime_utc)?)?;
    Ok(())
}

...
pub fn ssh_connect_with_ui(
    ssh_config: weenyterm_ssh::ConfigMap,
    ui: &mut ConnectionUI,
...
pub fn ssh_domain_to_ssh_config(ssh_dom: &SshDomain) -> anyhow::Result<ConfigMap> {
    ...
    ssh_config.insert(
        "weenyterm_ssh_backend".to_string(),
        match ssh_dom
            .ssh_backend
    ...
    if let Some("true") = ssh_config.get("weenyterm_ssh_verbose").map(|s| s.as_str()) {
...
fn connect_ssh_session(
    ...
    let renderer = termwiz_funcs::new_weenyterm_terminfo_renderer();
        ...
        let StartNewSessionResult { pty, child, writer } = if let Some(session) = session.take() {
                ...
                .context("request ssh pty")
            {
                ...
                Err(err) => {
                    if err
                        .root_cause()
                        .downcast_ref::<weenyterm_ssh::DeadSession>()
                        .is_some()
                    {
...
impl std::io::Write for PtyWriter {
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
        ...
        // the next byte.
        // <https://github.com/wez/weenyterm/issues/771>

...
impl Header {
    fn new(config: &ConfigHandle, size: PtySize, prog: &[&OsStr]) -> Self {
        ...
        env.insert(
            ...
            config::weenyterm_version().to_string(),
        );
        env.insert(
            "WEZTERM_TARGET_TRIPLE".to_string(),
            config::weenyterm_target_triple().to_string(),
        );
...
pub struct RecordCommand {
    ...
    /// the default_cwd defined by your weenyterm configuration
    #[arg(long)]
    cwd: Option<std::path::PathBuf>,

    /// Start prog instead of the default_prog defined by your
    /// weenyterm configuration
    #[arg(value_parser)]
    prog: Vec<OsString>,
}

...
`keys` section in your `~/.weenyterm.lua` config file.  For example, you can
...
config.keys = {
  ...
  -- be potentially recognized and handled by the tab
  {
    key = 'm',
    mods = 'CMD',
    action = weenyterm.action.DisableDefaultAssignment,
  },
}
```
 ...
 * `LEADER` - a special modal modifier state managed by `weenyterm`. See [Leader Key](#leader-key) for more information.
...
If you omit an explicit prefix, weenyterm will assume `phys:` and use the
...
In some cases, `weenyterm` may not know how to represent a key event in either
...
config.keys = {
  ...
  -- Send "CTRL-A" to the terminal when pressing CTRL-A, CTRL-A
  {
    key = 'a',
    mods = 'LEADER|CTRL',
    action = weenyterm.action.SendKey { key = 'a', mods = 'CTRL' },
  },
}
```

`weenyterm` allows assigning action(s) to specific key events, and comes
...
* *Physical Key* - a way to identify a key based on its hardware-dependent location. `weenyterm` can refer to keys based on code they would emit if configured to use an ANSI US English keyboard layout (even if that layout is not currently active), or based on its raw scan code.
...
And then some weenyterm concepts:

* *Key Assignment* - an action assigned to a matching key and modifier combination.
* *Key Table* - a grouping of key assignments. For each window, `weenyterm` maintains a stack of table activations, allowing for rich modal keyboard input customization


## Keyboard Processing Flow

This schematic depicts the processing flow for keyboard events in `weenyterm`:
...
a concept.  WeenyTerm tries to behave reasonably by default, but also give you
...
weenyterm will respect the composition effects of AltGr produced by the system.
...
WeenyTerm is now able to perform dead-key expansion when `use_ime = false`.  Dead
...
WeenyTerm has support for using the operating system Input Method Editor (IME) on
...
a dead key in weenyterm will "hold" the dead key until the next character is
...
You can tell WeenyTerm to disable dead keys by setting this in your configuration
file:
...
`weenyterm` to prevent it from doing that, short of disabling the IME.

### Defining Assignments for key combinations that may be composed

When a key combination produces a composed key result, weenyterm will look up

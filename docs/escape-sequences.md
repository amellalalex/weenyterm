...
>
> If you notice that something is inaccurate or missing, please do [file an issue](https://github.com/wez/weenyterm/issues/new/choose)
> so that it can be resolved!

## Output/Escape Sequences

WeenyTerm considers the output from the terminal to be a UTF-8 encoded stream of
...
WeenyTerm will attempt to apply your [font shaping](config/font-shaping.md) configuration
...
| ^G |0x07|BEL |Bell       |Logs `Ding! (this is the bell)` to stderr of the WeenyTerm process. See [#3](https://github.com/wez/weenyterm/issues/3)|
...
As mentioned above, WeenyTerm only supports UTF-8 encoding.  C1 control codes
...
WeenyTerm classifies these sequences into a number of functional families which
...
parameters be separated by colons (`:`), but for compatibility reasons WeenyTerm
...
`WeenyTerm` and most (all?) other terminal emulators:
...
This is a weenyterm extension: weenyterm considers color mode `6` as RGBA,
...
WeenyTerm supports [Synchronized Rendering](https://gist.github.com/christianparpart/d8a62cc1ab659194337d73e399004036).
...
|DCS \[PARAMS\] q \[DATA\] ST | Sixel Graphic Data | Decodes [Sixel graphic data](https://vt100.net/docs/vt3xx-gp/chapter14.html) and apply the image to the terminal model. Support is preliminary and incomplete; see [this issue](https://github.com/wez/weenyterm/issues/217) for status. |
|DCS 1000 q | tmux control mode | Bridges tmux into the WeenyTerm multiplexer.  Currently incomplete, see [this issue](https://github.com/wez/weenyterm/issues/336) for status. |
...
are canonically delimited by the `ST` (String Terminator) sequence, but WeenyTerm

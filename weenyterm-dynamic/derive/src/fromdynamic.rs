...
fn derive_struct(input: &DeriveInput, fields: &FieldsNamed) -> Result<TokenStream> {
    ...
    let bound = parse_quote!(weenyterm_dynamic::FromDynamic);
    ...
    let from_dynamic = match info.try_from {
        Some(try_from) => {
            quote!(
                ...
                <#ident>::try_from(target).map_err(|e| weenyterm_dynamic::Error::Message(format!("{:#}", e)))
            )
        }
        None => {
            quote!(
                match value {
                    Value::Object(obj) => {
                        weenyterm_dynamic::Error::raise_unknown_fields(options, #literal, &obj, Self::possible_field_names())?;
                        #obj
                    }
                    other => Err(weenyterm_dynamic::Error::NoConversion {
    ...
    let tokens = quote! {
        ...
        const #dummy: () = {
            impl #impl_generics weenyterm_dynamic::FromDynamic for #ident #ty_generics #bounded_where_clause {
                fn from_dynamic(value: &weenyterm_dynamic::Value, options: weenyterm_dynamic::FromDynamicOptions) -> std::result::Result<Self, weenyterm_dynamic::Error> {
                    use weenyterm_dynamic::{Value, BorrowedKey, ObjectKeyTrait};
                    #adjust_options
                    #from_dynamic
                }
...
fn derive_enum(input: &DeriveInput, enumeration: &DataEnum) -> Result<TokenStream> {
                        ...
                        quote!(
                            #literal => {
                                match value {
                                    ...
                                    other => return Err(weenyterm_dynamic::Error::NoConversion {
                                        source_type: other.variant_name().to_string(),
                                        dest_type: "Object",
                                    }),
                                }
                            ...
                            let var_fields = fields
                                ...
                                .map(|(idx, f)| {
                                    ...
                                    quote!(
                                        <#ty>::from_dynamic(
                                            arr.get(#idx)
                                            .ok_or_else(|| weenyterm_dynamic::Error::Message(
            ...
            quote!(
                    match value {
                        Value::String(s) => {
                            match s.as_str() {
                                #( #units )*
                                _ => Err(weenyterm_dynamic::Error::InvalidVariantForType {
                        ...
                        Value::Object(place) => {
                            if place.len() == 1 {
                                ...
                                Err(weenyterm_dynamic::Error::IncorrectNumberOfEnumKeys {
                                    type_name: #literal,
                                    num_keys: place.len(),
                                })
                            }
    ...
    let tokens = quote! {
        #[allow(non_upper_case_globals)]
        const #dummy: () = {
            impl weenyterm_dynamic::FromDynamic for #ident {
                fn from_dynamic(value: &weenyterm_dynamic::Value, options: weenyterm_dynamic::FromDynamicOptions) -> std::result::Result<Self, weenyterm_dynamic::Error> {
                    use weenyterm_dynamic::{Value, BorrowedKey, ObjectKeyTrait};
                    #from_dynamic
                }

...
fn derive_struct(input: &DeriveInput, fields: &FieldsNamed) -> Result<TokenStream> {
    ...
    let bound = parse_quote!(weenyterm_dynamic::PlaceDynamic);
    ...
    let tokens = match info.into {
        Some(into) => {
            ...
            const #dummy: () = {
                ...
                impl #impl_generics weenyterm_dynamic::ToDynamic for #ident #ty_generics #bounded_where_clause {
                    fn to_dynamic(&self) -> weenyterm_dynamic::Value {
        ...
        None => {
            ...
            #[allow(non_upper_case_globals)]
            const #dummy: () = {
                impl #impl_generics weenyterm_dynamic::PlaceDynamic for #ident #ty_generics #bounded_where_clause {
                    fn place_dynamic(&self, place: &mut weenyterm_dynamic::Object) {
                        #(
                            #placements
                        )*
                    }
                ...
                impl #impl_generics weenyterm_dynamic::ToDynamic for #ident #ty_generics #bounded_where_clause {
                    fn to_dynamic(&self) -> weenyterm_dynamic::Value {
                    use weenyterm_dynamic::PlaceDynamic;

                    let mut object = weenyterm_dynamic::Object::default();
                    self.place_dynamic(&mut object);
                    weenyterm_dynamic::Value::Object(object)
                    }
                }
            };
                )
    ...
    let tokens = match info.into {
        Some(into) => {
            quote! {
                #[allow(non_upper_case_globals)]
                const #dummy: () = {
                    impl weenyterm_dynamic::ToDynamic for #ident {
                        fn to_dynamic(&self) -> weenyterm_dynamic::Value {
                            let target : #into = self.into();
                            target.to_dynamic()
                        }
                    ...
                    Fields::Named(fields) => {
                        ...
                        Ok(quote!(
                            Self::#ident { #( #var_fields, )* } => {
                                let mut place = weenyterm_dynamic::Object::default();

                                #( #placements )*

                                let mut obj = weenyterm_dynamic::Object::default();

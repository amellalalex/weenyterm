...
#flatpak run --env=G_DEBUG=fatal-criticals org.freedesktop.appstream-glib validate assets/weenyterm.appdata.xml
...
if [ "${CI}" != "yes" ] ; then
  flatpak-builder \
    --state-dir /var/tmp/weenyterm-flatpak-builder \
    --install /var/tmp/weenyterm-flatpak-repo \
    assets/flatpak/org.weenyfurlong.weenyterm.json \
    --force-clean --user -y
fi
